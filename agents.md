...
2. **Повний обхід політик без Root і без патчингу системних файлів**:
   - Захоплення працює через стандартний Android API `AudioRecord`, не потребує Magisk-модулів чи патчів (достатньо звичайного `RECORD_AUDIO`).
   - AudioFlinger коректно мультиплексує доступ, тому паралельні звернення асистента («Окей Google») чи дзвінки не ламають ядро і не ловлять `EBUSY` (як при спробах прямого відкриття сирих ALSA-нод `/dev/snd/pcmC0D0c` чи `pcmC0D2c`).
3. **Прорив для автокалібрування салону (RTA / Room Measurement)**:
   - У системі є офіційний, чистий, повнодіапазонний канал для вимірювання резонансів салону, фазових затримок та калібрування 16-смугового EQ під реальну передатну функцію автомобіля без необхідності підключати зовнішні USB-мікрофони!
