...
3. **Метод аналізатора на самому записі свіпу** (енергія третинооктавних смуг, згортка як у натива, вікно
   ...
   **низ дверей — свіп ЗАВИЩУЄ**: 20 Гц +29…+35, 31.5 Гц +15…+16, 50 Гц +6…+7; **саб** на 31.5–80 Гц у −2…+2
   (20 Гц +15). **Механізм знайдено → `1369335`:** вікно ІХ відкривалось різко за 64 відліки до НАЙБІЛЬШОГО піку, а це відбиття від панелі, не перший звук — зріз ішов через прямий звук і заливав низ. Модель numpy відтворює звіт до ±0.5 дБ; вікно за 100 мс із плавним підйомом → 31.5–125 Гц сходиться з аналізатором у 2–3 дБ, вище 125 Гц — без змін. Новий хост-тест «прямий звук + відбиття через ФВЧ 150 Гц» падав на старому вікні (14.8 замість ~26 дБ). Побічне: смуга 20 Гц на рівному тракті тепер −6.5 дБ, як 20 кГц (−6.6) — обидва краї свіпу напівзбуджені; калібрування мікрофона тим самим методом це скорочує; окремий пункт.
...
### 🎚️ Краї свіпу (15.09, ніч)
- **`2d5717c`**: смуга рахується лише по збуджених свіпом бінах → на рівному тракті всі 16 смуг у 0.3 дБ (свіп
  застосунку 6 с, 20 Гц–20 кГц); вада «20 кГц −6.6» закрита без зміни діапазону.
- ▶️ **Наступне (наказ власника): верх свіпу — з проби КАНАЛУ, не з припущення.** Заводські політики можуть
  видавати в I2S 44.1 кГц (BitPerfect — справжні 24/48; заводські «ніби точно 16/48»). Ідея: перед свіпом два
  тихі тони через один канал по обидва боки 22.05 кГц (напр. 21 і 23 кГц), запис 48 кГц — стіна ресемплера
  = десятки дБ, спад твітера/мікрофона = кілька; проходить → свіп до ~23.5 кГц, ні або не видно → 20 кГц і
  сказати про це в звіті. `PROPERTY_OUTPUT_SAMPLE_RATE` — декларація мікшера, не дріт. Потребує звуку → після ночі.
- Пам'ять `sweep-top-from-channel-probe`. ⚠️ Калібрування мікрофона й криві салону, зняті до `1369335`/`2d5717c`,
  несуть старий низ і старі краї — переміряти (борг тестів).
### 🧾 Ніч 15.09 — підсумок (усе зібрано debug, на апарат НЕ ставилось; нічого не запушено)
| коміт | що |
|---|---|
| `b920941` `3da5c49` | замок автопресетів знято + примітка без рута; рут — один клас без префа й опитування |
| `6cc329f` | вимір питає, ХТО відкрив вхід мікрофона, а не лише його ширину (з рутом зганяє, без — ребут) |
| `b5951da` | двері салону усереднюються по потужності (промах 12.5/20 кГц +11/+19 → +1.4/+2.1) |
| `1369335` `2d5717c` | вікно ІХ свіпу відкривається до першого приходу (низ дверей); краї смуг — лише збуджені біни (усі 16 смуг у 0.3 дБ) |
| `7cb210e` `a6dd6a0` | перевірка дослідження динамічного басу + реверс Дж: саб живиться ПІСЛЯ EQ |
| `b573fec` | логер: перші 20 рядків кожної форми за 10 с, решта підсумком (шторм 42 769 → 2 217) |
| `1d9ca25` | одна таблиця HPF/саба: код 0 = Through, не «20 Гц» |

**❓ Питання власнику (не вирішено):**
1. «Компенсація саба» подвоює підйом (EQ до кросовера): лишити / прибрати / давати саб лише те, що EQ не дав через стелю +12 дБ.
2. Шкала сітки EQ проти спектра («0» = −30 dBFS, поділка 2.5 дБ) і чи потрібні перемикачі нормалізації головного.
3. Низ свіпу нижче 20 Гц: скретч-пресет пускає двері без ФВЧ — свіпити з ~15 Гц чи лишити 20 Гц (краї вже правильні).

**▶️ Далі (потрібен звук):** проба каналу для верху свіпу; переміряти калібрування мікрофона і салон (старі криві несуть старий
низ і краї); розовий шум на скретч-пресеті на гучності виміру (умови живого низу); ціна кроку гучності в дБ; акустична проба
«+6 дБ на 31.5 Гц → чи виріс саб»; усе з «Борг тестів» вище. Логер `out/wDSP_bootlog.zip` — поставити, коли власник скаже.
### ✅ Відповіді власника на три питання (15.09.2026)
1. **«Компенсація саба» — лишити як є.** «Кнопку залиш, вона автору оригінала потрібна як пам'ять, і може комусь мало
   баса. Я до речі її сам вмикав.» Подвійний підйом через EQ до кросовера — відомий і свідомий: це додатковий бас.
2. **Спектр відносно власного середнього в дБ еквалайзера + авторівень головного прибрати** → `74680a7`.
3. **Свіп: двері з 20 Гц, саб із 15 Гц** → `d32129e`.
### ▶️ Автовідновлення плеєра — `9ee8368` (15.09.2026), наказ 14.09 «два вмикачі»
- `PlayerResume`: знімок «що грало» на `READY_GO_SLEEP`/`ACC_OFF` (пауза ≤5 с до — платформна), відновлення через 4 с після
  `ACC_ON` (вмикач «після сну») або 5 с після готових пресетів у перші 3 хв від старту ядра (вмикач «після
  перезавантаження»): сесія → PLAY у media-button-приймач → media browser; через 6 с лог «is playing / NOT playing».
- Не чіпає: радіо, паузу людини, коли вже щось грає (і Jitu2-лаунчер), п'ять заводських із `RestoreAppsWhenWakeup.ini`.
- Вмикачі в «Дозволи та система», за замовчуванням вимкнені. Переклади 28 локалей — Дж (#545).
- ❓ Білий список сну (`/great/sleep/sleep_whitelist`): як застосунку перевірити членство і відкрити екран додавання — питання Дж
  `60ce423d` (#546), відповідь у `.agents/RESEARCH_SLEEP_WHITELIST.md`. Самим не додаватися.
- 🧾 Борг тестів (звук): YT Music грає → вимкнути запалювання → увімкнути; те саме з паузою; з радіо; ребут. Логер: тег `wDSP_PlayerResume`.

## 🔴 15.09.2026, ранок — ЗРІЗ СЕСІЇ (нічний; ДІЙСНИЙ у частинах «Доведено», «Під питанням», «Борги коду», «Сесії Дж» — новіший зріз 15.09 ~07:00 у кінці файла читати першим)

> **Правило зрізу (власник):** читати документи, а не переміряти відоме. Усе нижче або доведено (з доказом), або
> прямо позначено «під питанням». Що доведено — на апараті НЕ повторювати.

### 🔴🔴 НАГАЛЬНИЙ ДЕННИЙ БОРГ — ЗВУК (власник, 15.09.2026 ніч: «ніч — ніяких проб звуку не буде, відклади як нагальний борг в денний час»)
**Вночі нічого з цього не робити. Уранці — першим, раніше за будь-який новий код.**
Жоден із 36 незапушених комітів на апараті не бачено: там досі збірка вечора 14.09.
1. **Поставити на апарат** поточну збірку і новий логер-модуль. Після перевстановлення мікрофон забирає асистент: з рутом
   застосунок забере сам, без рута — ребут.
2. **Перевірити на апараті** (рядки логера для кожного — у таблиці тестів нижче):
   - рут: Magisk питає один раз на старті; переходи між екранами — жодного тосту;
   - вимір салону відкривається без замка;
   - мікрофон: перше вмикання; випадок, коли на вході вже хтось є;
   - відносний спектр: розовий шум на рівному пресеті стоїть біля нуля, два тони — порожньо;
   - у Налаштуваннях немає «авторівня головного» і «динамічної нормалізації»;
   - **заново відкалібрувати мікрофон** (старе калібрування зняте кривим вікном) → новий вимір салону → розовий шум проти кривої;
   - **чому бас на 31.5 Гц тихший на 19 дБ**: розовий шум на рівному пресеті, гучність 16;
   - автовідновлення плеєра: після сну, після ребуту, пауза людини, радіо, щось уже грає;
   - кнопка білого списку сну: чи відкривається заводський екран без пароля;
   - логер під час шторму HAL: підсумкові рядки є, рядки wDSP не губляться;
   - радіо в режимі мікрофона; дзвінок BT одразу гасить заставку;
   - ціна одного кроку гучності в дБ (тон 1 кГц, кроки 1…32);
   - чи саб отримує звук після еквалайзера: підняти смугу 31.5 Гц і слухати саб.
3. **Проба каналу** — чи тракт пропускає звук вище 22 кГц (заводські можуть віддавати в I2S 44.1 кГц). Код не почато;
   відкладено разом зі звуком. Доти свіп салону — до 20 кГц.

Закривати пункт лише доказом з апарата (рядок логера, запис, вимір), не наміром.

### 0. Стан на момент зрізу
- Гілка `kostyfmat_mod`. **Не запушено 36 комітів** (`origin/kostyfmat_mod` = `002c367`, HEAD = `8abca6b`, сам зріз — 36-й). Пуш — лише за
  наказом, з WSL від `user`.
- **На апараті стоїть збірка `7a58df0` (14.09).** Нічого з цієї сесії не встановлено. Логер-модуль на апараті — старий
  (новий zip зібрано: `tools/wdsp_bootlog_module/out/wDSP_bootlog.zip`, не встановлено).
- Останній boot на апараті — 0009. У Magisk рут для wDSP дозволено (policy 2); у старій збірці `pref_root_granted=false`
  (нова збірка мігрує це сама — див. §2).
- Усі збірки цієї сесії — `assembleDebug`, пройшли. `tools/check_locales.py` — 0 проблем. Хост-тести свіпу й аналізатора — ALL PASS.

### 1. Рішення власника цієї сесії (дослівно — не перепитувати)
| тема | слова власника | що це означає |
|---|---|---|
| Замок автопресетів | «замок зняти в акордионі… люди без рут повинні бути чітко інформовані… програма попросить ребут, якщо мікрофон не наш» | зроблено `b920941` |
| Рут | «Записувати стан рута в преф — погана ідея… перевіряти рут при першому вмиканні мікрофона, при першому старті після перевстановлення… пуллінг рут прав викликає тоаст… джерелом правди один метод або максимум клас… перевіряти ще під час завантаження» | зроблено `3da5c49` |
| Перший промпт Magisk | відповідь на питання: «Перше питання — від дії» | на старті `su` лише якщо Magisk уже відповідав цьому встановленню |
| Ніч | «тестів звуку не буде, ніч… робиш те, що далі по плану, а тести відкладаєш в борг» | усе нижче не бачено на апараті |
| Переклади | «перекладами у нас ДЖ займається, і я заєбався просити віддавати рутину йому» | 28 локалей — завжди Дж (сесія `931e1b40`) |
| Верх свіпу | «хто забороняє свіпити вище?… фабричні політики можуть впертися в 44 на видачу в i2s… Програма повинна пробувати канал, а не робити поспішні висновки… фабричні ніби точно вміють 16\48» | верх 20 кГц, поки не зроблено пробу каналу |
| Компенсація саба | «кнопку залиш, вона автору оригінала потрібна як пам'ять, і може комусь мало баса. Я до речі її сам вмикав» | лишається як є, подвійний підйом — свідомий |
| Шкала спектра | «В, авторівень прибрати» | зроблено `74680a7` |
| Низ свіпу | «лишай 20 для динаміків і 15 для саба» | зроблено `d32129e` |
| Автовідновлення плеєра | «може зробиш щось із туду, типу запуск плеєрів і два вмикача?» | зроблено `9ee8368` + кнопка білого списку `c246e9c` |
| Довідник Дж | «та ж сесія Дж це вже знала» (про `18-MEDIA-SESSION-AND-SLEEP-RESUMPTION.md`) | перед платформною функцією — спершу `references/` сесії `60ce423d` |

### 2. Зроблено (усе зібрано, на апараті НЕ бачено, якщо не сказано інше)
| коміт | що | як перевірено |
|---|---|---|
| `b920941` | секція виміру салону відкрита без рута (два замки: акордеон + питання на «Старт»); примітка `desc_room_no_root` (видно лише без рута); 5 мертвих ключів `room_root_*` знесено | збірка, locales 0 |
| `3da5c49` | `RootAccess` — одне джерело: `hasRoot()` у пам'яті, `su` лише `checkAtStart` (раз за процес, лише якщо в `wdsp_device_state` `magisk_answered`), `checkForMicrophone` (перше вмикання мікрофона людиною), `request` (картка). Опитування на resume/wake/відкритті майстра прибрано. Легасі `pref_root_granted` → мігрує в `magisk_answered=true` | збірка |
| `6cc329f` | вимір питає, **хто відкрив вхід мікрофона** (не лише ширину): чужий першим → з рутом зганяє й перевіряє, без рута — `needsRestart` | збірка |
| `b5951da` | двері салону усереднюються **по потужності** (було в дБ) | офлайн на числах звіту 13.09 + дампи 14.09 (§3) |
| `1369335` | вікно ІХ свіпу відкривається за 100 мс до приходу з плавним підйомом (було різко за 64 відліки) | numpy-модель + хост-тест (§3) |
| `2d5717c` | смуга рахується лише по збуджених свіпом бінах → краї 20 Гц/20 кГц рівні | хост-тест: 16 смуг у 0.3 дБ |
| `d32129e` | свіп саба від 15 Гц, двері від 20 Гц, один прохід | хост-тест: 16 смуг у 0.7 дБ |
| `7cb210e` `a6dd6a0` | перевірка дослідження динамічного басу; реверс Дж про джерело кросовера саба | §3 |
| `b573fec` | логер: перші 20 рядків кожної «форми» за 10 с, решта — підсумок `wDSP_bootlog: N more lines like this…`; теги wDSP не чіпає | шторм boot 0005: 42 769 → 2 217 на трьох awk |
| `1d9ca25` | одна таблиця HPF/саба (`DspResponse`): код 0 = Through, не «20 Гц» | збірка |
| `74680a7` | головний спектр **відносний**: смуги в дБ повзунків відносно середнього смуг зі звуком; «Авторівень: головний» і «Динамічна нормалізація (AGC)» прибрано; головний споживач без підсилення | збірка |
| `9ee8368` `156e621` | `PlayerResume` + два вмикачі «після перезавантаження» / «після сну» (за замовчуванням вимкнені); 30 локалей | збірка, переклад Дж перевірено |
| `c246e9c` `8ed74bb` | кнопка «Дозволити wDSP жити під час сну» → `com.qf.carsettings/.activity.FactorySleepWhiteListActivity` (лише де резолвиться); 30 локалей | збірка |
| `CLAUDE.md` | абзаци: рут, відносний спектр, автовідновлення, свіп | — |

### 3. Доведено (НЕ переміряти)
1. **Крива салону в дБ-середньому була хибна.** Звіт 13.09 19:51 (той, з якого `pref_cabin_response`, відтворено до
   0.04 дБ): FL −32.7/−50.5 дБ на 12.5/20 кГц, FR −5.5/−9.3. Після усереднення по потужності промах живого розового шуму
   8 кГц +3.9→+0.9, 12.5 кГц +11.4→+1.4, 20 кГц +19.1→+2.1; 315 Гц–20 кГц у ±2.6 (крім 3150 +3.6). Пам'ять `cabin-average-in-power-not-db`.
2. **Метод свіпу завищував глибокий низ дверей** (20 Гц +29…+35 дБ, 31.5 +15, 50 +6) через різкий зріз вікна за 1.3 мс
   до найбільшого піку (пік — відбиття від панелі, не перший звук). numpy-модель відтворює звіт до ±0.5 дБ; вікно за
   100 мс → 31.5–125 Гц сходиться з методом аналізатора в 2–3 дБ; середина й верх — без змін. Хост-тест «прямий звук +
   відбиття через ФВЧ 150 Гц»: старе вікно 14.8 дБ (FAIL), нове 24.7 (теорія ~26).
3. **Саб живиться ПІСЛЯ еквалайзера.** `mcu.bin`: індекс 43 = регістр `0206`, 28 = `0102` (перевірено); даташит:
   `0206` після скидання = `00` → `SL X'over Input` = Time Alignment (основний ланцюг). `RESEARCH_BU32107_SUB_SOURCE.md`.
4. **Твердження дослідження Дж спростовано нашими даними:** «2 команди/с» — це наш коментар `THROTTLE_MS`; зрізу
   мікрофона на 70–150 Гц немає (саб на 31.5 Гц чутно з SNR ~20 дБ). `RESEARCH_RESULT_DYNAMIC_BASS.md`, таблиця вердиктів.
5. **Білий список сну:** `/great/sleep` `drwx------ system`, файл `-rw------- system`, 94 байти: `com.navioverlay.car`,
   `com.radiorubka.wdsp`, `com.huautobrightness.controller`, `com.kostyamat.fmradio`. Застосунок його прочитати не може.
   `com.qf.carsettings/.activity.FactorySleepWhiteListActivity` існує з фільтром VIEW. `RESEARCH_SLEEP_WHITELIST.md`.
6. **Шторм HAL** (boot 0005): ~110 спроб/с по 27 рядків, `cannot open pcm … Device or resource busy`, 20 МБ ротації за ~20 с.
7. **Живий розовий шум на низу тихіший за запис свіпу тим самим мікрофоном** (≈ −19 дБ на 31.5 Гц, −10 на 50, −4…−5 на
   80–125). Це факт спостереження; причина — §4.
8. Раніше (14.09, не переміряти): Visualizer 48 кГц; стібер по годиннику; перші на вході → AEC/NS не діють (тон ±0 дБ);
   без рута мікрофон тримається від старту; (б) зсув +2.1 дБ; `sys.qf.last_audio_src` липкий після ребуту.

### 4. Під питанням (не доведено — не видавати за факт)
1. **Чому живий низ тихіший** (§3.7): пресет «wDSP Flat» проти скретч-пресету, гучність 8 проти 16 (+ тонкомпенсація),
   тракт BitPerfect — не розділено.
2. Залишки після виправлень: 3150 Гц +3.6; 125–200 Гц −5.
3. **Чи пише прошивка `0206` під час роботи** — Дж сказав «ніхто», але його таблиця команд UART хибна (0x80 = EQ, 0x89 = surround,
   0x8B = саб); вирішальна проба — акустична (§5, T15).
4. **Крок гейна саба** — 1 дБ чи 2 дБ на індекс: `DspResponse` і `LoudnessCurve.subOffset` вважають дБ = індекс; не звірено.
5. **Заводський I2S 44.1 чи 48 кГц** — власник «ніби 16/48»; проби каналу немає (§6).
6. **Автовідновлення:** порядок `READY_GO_SLEEP`/`ACC_OFF`; чи платформа ставить паузу до них (грація 5 с — припущення);
   чи холодний старт YT Music (morphe) іде через media button / browser; чи Jitu2-лаунчер не випередить.
7. **Екран білого списку** відкривається без пароля при прямому запуску — зі слів Дж, не перевірено.
8. **Ризик `6cc329f`:** якщо на вході завжди є якийсь запис, кожен свіп без рута проситиме ребут.
9. **Magisk:** чи тайм-аут промпту зберігає «deny» (тоді `magisk_answered=false` і на старті не питаємо — так і задумано).
10. Калібрування мікрофона власника (`+29.9` на 20 Гц) знято старим вікном — ймовірно спотворене; переміряти.
11. Шкала «Діапазон» (60 дБ) у відносному спектрі впливає лише на поріг тиші — чи зручно, скаже око власника.

### 5. Борг тестів на апараті (по порядку; у дужках — що шукати в логері `bootlog/<latest>/`)
**Встановлення:** поставити поточну debug-збірку (HEAD). ⚠️ Перевстановлення віддає мікрофон асистенту — з рутом застосунок
забере сам, без рута — ребут. Логер-модуль встановити разом (`magisk --install-module`), діє з наступного завантаження.
1. **T1 старт/рут:** `wDSP_RootAccess: start: GRANTED` один раз за старт; тост Magisk — один; `root: the microphone is opened when wanted`.
2. **T2 без опитування:** повернення в Налаштування/головний екран — жодного тосту Magisk.
3. **T3 секція салону:** відкривається; з рутом примітки «Без рута…» немає.
4. **T4 мікрофон:** перемкнути спектр на «мікрофон» → `first microphone use: GRANTED`; з чужим на вході — `input holders read from
   AudioFlinger`, `… is running again`.
5. **T5 відносний спектр:** розовий шум на Flat → смуги біля «0»; звичайний трек → форма; тест двома тонами → смуг немає (<3 смуг зі звуком, так задумано).
6. **T6 Налаштування:** рядків «Авторівень: головний» і «Динамічна нормалізація» немає; авторівень віджета на всю ширину.
7. **T7 калібрування мікрофона → вимір салону** (новий свіп): у звіті `from 20 Hz (subwoofer from 15 Hz)`; двері на 20/31.5 Гц
   тепер низько, саб — ні; потім розовий шум (мікрофон−розрахунковий проти кривої салону) → очікування ±3 дБ на 315 Гц–20 кГц.
8. **T8 умови низу:** розовий шум на пресеті як скретч (EQ рівно, HPF Through, саб 0, loud/fm вимк.) і гучності 16 — чи зникає −19 дБ на 31.5 Гц.
9. **T9 вхід чужий:** свіп, коли інша програма відкрила вхід першою → у звіті `opened by it before us`; без рута — прохання ребуту.
10. **T10 автовідновлення** (обидва вмикачі ввімкнути; тег `wDSP_PlayerResume`): (а) YT Music грає → запалювання вимк./увімк. →
    `after sleep: asked … to play through …`, `… is playing`; (б) пауза руками → `was paused before the ignition went off`;
    (в) радіо → `radio - it restores itself`; (г) ребут під час гри → `after restart: …`; (ґ) щось уже грає → `something is already playing`.
11. **T11 кнопка білого списку:** дотик → заводський екран без пароля.
12. **T12 логер:** у `20_run.log` є `wDSP_bootlog: N more lines like this in the 10 s window …` під час шторму; рядки wDSP без пропусків.
13. **T13 давні:** радіо в режимі «мікрофон»; дзвінок BT — заставка зникає одразу, спектр на паузі.
14. **T14 ціна кроку гучності в дБ:** тон 1 кГц, кроки 1…32, мікрофоном (для кривої тонкомпенсації).
15. **T15 саб після EQ акустично:** скретч-пресет, +6 дБ на смузі 31.5 Гц → чи виріс вихід саба.
16. **T16 проба каналу** — після того, як її буде написано (§6).

### 6. Борги коду й дизайну (не тести)
1. **Проба каналу для верху свіпу** (наказ власника): два тихі тони по обидва боки 22.05 кГц через один канал, запис 48 кГц;
   стіна ресемплера = десятки дБ, спад твітера/мікрофона = кілька; проходить → свіп до ~23.5 кГц, ні → 20 кГц і сказати у звіті.
2. P2Bass не змодельовано в `DspResponse`.
3. Метадані системних плеєрів у заставці: протокол уже в базі Дж (дошка #539, `references/08-VOLUME-AND-SOURCES.md §6`).
4. Межа AGC віджета −30 dBFS — око власника.
5. Звірити крок гейна саба (§4.4).
6. Пуш 35 комітів — за наказом.

### 7. Сесії Дж на цій машині — хто за що (питати ТІЄЇ, що знає; спершу читати її довідники)
| сесія (дошка) | ділянка | коли звертатися | де її знання |
|---|---|---|---|
| **`931e1b40`** (= `gemini_ui_dev`, «wDSP UIX Dev»; cwd wDSP, пісочниця `d:\gemini\wdsp_test`) | **рядки й локалі wDSP (28 мов)**, UI-уніфікація, стара мікрофонна математика | будь-який новий рядок: я пишу `values` + `values-uk`, їй — завдання `strings-*` з абсолютними шляхами, «git не чіпати», `check_locales` до 0 | дошка, topic `strings-*`; цієї сесії: #545/#549, #551/#553 |
| **`60ce423d`** | **прошивка й фреймворк QF**: декодування HW-коду MCU, реверс регістрів (`0206`), сон і відновлення плеєрів, білий список сну, метадані віджета лаунчера | платформне питання або реверс; **спершу** читати її довідники | `C:\Users\kosty\.gemini\config\skills\qf-platform-architecture\references\` (напр. `18-MEDIA-SESSION-AND-SLEEP-RESUMPTION.md`, `01-SYSTEM.md`, `08-VOLUME-AND-SOURCES.md`); цієї сесії: #540/#542, #546/#550 |
| `6a1bdb9a` (через MCP `ask_antigravity_assistant`) | старий реверс MCU / Ghidra | лише якщо дошка мовчить | журнал `.agents/MCU_REVERSE.md` (радіо) |
| `d7e3c03a` | джерела Unisoc Android 10 BSP | за потреби BSP | — |
| `dc97126f` / `logo-pack-repair-05-09` | лого-паки радіо | не для wDSP | — |
| `a326d4ed`, `mcp-bridge-dev` | сам agent-bridge | не для wDSP | — |
| `36b08553`, `6ecb8a68` | стара локалізація через MCP; HW_CODE звіт радіо | не використовувати | — |

⚠️ Відповідь Дж — квитанція, не доказ: звіряти файлами (`git diff --numstat`, `check_locales`) і апаратом. Цієї сесії одна його
таблиця (команди UART) виявилась хибною, решта підтвердилась.

### 8. Помилки сесії (не повторювати)
- Сам перекладав 30 локалей — це робота Дж (докір третій). Пам'ять `delegate-to-antigravity-gemini`.
- Писав автовідновлення з листа на дошці, не прочитавши довідник `60ce423d` — власник: «та ж сесія Дж це вже знала».
- Одразу поставив верх свіпу 23.5 кГц без проби каналу — власник зупинив.
- Прочитав відповідь «1 — файл» як «прибрати кнопку», хоча власник хотів лишити — перепитати тлумачення, коли відповідь не А/Б/В.
- `cd` у Bash міняє робочу теку сесії — абсолютні шляхи; для `adb push`/`wsl` з Git Bash — `MSYS_NO_PATHCONV=1`.

### 9. Інструменти й артефакти
- Скретчпад сесії `…\0079b58e-9830-44d5-bdef-a86693890553\scratchpad\`: `run_sweep_test.sh` (хост-тест свіпу у WSL),
  `run_harness.sh` (аналізатор), `test_ratelimit.sh`/`test_ratelimit2.sh` (awk логера), `storm_0005_run2.log`, `logger_grep.sh`.
- Запис свіпу 13.09 і звіт: скретчпад `b15c7e24…\scratchpad\last_meas\room_measurement.{wav,txt}`; дампи розового шуму 14.09:
  `b15c7e24…\scratchpad\exp\` (`calc.log`, `mic2.log`, `noise2.log`), скрипт `compare_modes.py` (`MEDIAN=1`).
- Нові документи: `.agents/RESEARCH_BU32107_SUB_SOURCE.md`, `.agents/RESEARCH_SLEEP_WHITELIST.md` (обидва в `INDEX.md`).
- Нова пам'ять: `state-15-09-wdsp-snapshot`, `cabin-average-in-power-not-db`, `sweep-top-from-channel-probe`,
  `owner-decisions-15-09-spectrum-scale-sweep-start`, оновлено `root-pref-not-refreshed-after-magisk-regrant`,
  `delegate-to-antigravity-gemini`, `gemini-knowledge-map`.

## 15.09.2026, 05:50–06:45 — скарги тестера з вертикальною панеллю: заставка і нижній рядок еквалайзера

**Звідки.** Тестер, K706 на UIS8581 (`uis8581a2h10`), вертикальна панель 768×1024, вікно застосунку 768×710dp (зверху
бар 160 px, знизу клімат 154 px), збірка 0.4.9.8. Фото + `wdsp_system_report_v0.4.9.8_vc25_20260914_194445.txt` +
«топологія» (Unigram `LocalState\0\documents\`). Власник: «заставка просто не з'являється — це не одна скарга».

**Заставка — причину НЕ знайдено, зроблено діагностику (`d1c85d9`).**
- На апараті власника працює (п'ять показів за ніч у логері). Отже блокує щось на апаратах тестерів.
- У звітах тестерів не було жодного поля, що могло б це сказати. Тепер блок SCREENSAVER є у звіті системи й у
  «Діагностика → Топологія екрана»: кожна умова зі значенням, найдовший простій проти потрібного, скільки разів і чим
  стримано, коли востаннє показано, останні 12 дотиків до заставки (координати у вікні й на екрані, розмір вікна й
  його місце, числа, з яких рахуються зони, і рішення). Перевірено на апараті: тап вище середньої лінії записано.
- ❓ Підозра, не доказ: `persist.sys.float_navi_bar` і `persist.sys.has.float.video` код читає як «вікно зараз
  відкрите», а в декомпільованому фреймворку це галочки заводських налаштувань (`ConfigInfoConstant.SET_CHECKBOX_NAME_MAP`).
  Де функцію ввімкнено — заставка заблокована назавжди. Поведінку не змінено до рішення власника; звіт тестера покаже.
- Що попросити в тестерів: лишити апарат на будь-якому додатку (не wDSP) 2+ хв, тоді «Топологія екрана» → зберегти.

**Нижній рядок еквалайзера — виправлено (`c41812d`).**
- Причина виміряна емуляцією (`wm size` + `dumpsys activity top -a`): блок «Підсилення» мав вагу 2.2, підпис міряється
  першим і забирає свою ширину, кнопкам — залишок. Дано/треба: 1024 — 222/281 dp, 768 — 149/236, 640 — 101/236.
- Власник: «все і зразу зменшувати, підпис жати кегль, і трохи шрифт»; «Гц встає другим рядом — менший шрифт, або без
  пробілу, або прокрутку». Зроблено: блок підсилення без ваги; `ui/RowFit` стискає весь рядок одним множником
  (підписи й частота повністю, решта тексту наполовину, повзунок ≥ 64dp; стрілка списку 52dp не стискається);
  частота `singleLine` + `TextScroller`. Після: 1280 без змін, 1024 повний розмір, 800 ×0.98, 768 ×0.94, 640 ×0.73, «100 Гц» цілий.
- Пілюля на 640: підписи й бічні відступи вкладок стискаються разом, «Налаштування» цілий; 1280 без змін.
- На фото тестера пілюлю закриває смужка візуалізатора — він сам опустив її (`sb_vis_offset_y` = 804 px).

**Заставка — причину ЗНАЙДЕНО й виправлено (06:40–06:55).**
- Доказ на апараті власника: галочка «плаваюче відео» в заводських налаштуваннях → `persist.sys.has.float.video=true`
  одразу, вікна відео немає, лаунчер спереду, заставка з тієї секунди стримана щотіку. Де галочку ввімкнено — заставки
  немає ніколи. `float_navi_bar` — той самий тип галочки у фреймворку. `sys.qf.navi_state` стримував над будь-яким додатком.
- Власник: навігатор за замовчуванням — це проп (`persist.sys.maps`, у власника `com.waze`), і його пакет у
  `sys.qf.current.activity` навіть у вікні лаунчера (перевірено: обидва resumed, проп = Waze).
- Зроблено (`c8cf5d9`, `a0745b6`): три перевірки прибрано; навігатор за замовчуванням додається у список
  винятків заставки **один раз за встановлення** (позначка в `wdsp_device_state`, поза бекапами) — людина може прибрати.
  Перевірено: з увімкненою галочкою заставка з'явилась над плеєром через 60 с; `com.waze` додано в список.
- В анотацію наступної роздачі — пам'ять `wdsp-next-release-notes-pending`.

## 🔴🔴 15.09.2026 ~07:00 — ЗРІЗ СЕСІЇ (читати ЦЕ першим)

> Правило власника: **читати документи, не переміряти відоме.** Нижче або доведено з доказом, або прямо «під питанням».
> Нічний зріз вище («15.09.2026, ранок») лишається дійсним у своїх частинах «Доведено», «Під питанням», «Борги коду»
> і «Сесії Дж» — тут вони не повторюються, лише доповнюються.
> 🗣️ З власником говорити **словами про річ**, без «§5», «T7», «пункт 4.2» (докір 15.09 ніч, пам'ять `name-the-thing-not-the-index`).

### Стан на момент зрізу
- Гілка `kostyfmat_mod`, HEAD — коміт цього зрізу. **Не запушено 47 комітів** (`origin/kostyfmat_mod` = `002c367`). Пуш —
  лише за наказом, з WSL від `user`. Дерево чисте, крім двох файлів `.idea`.
- **На апараті власника — збірка з кодом `c2e46ef`** (поставлена 06:56 стрімом поверх, debug). Логер-модуль **v2** (з
  обмеженням повторів) поставлено й ребутнуто о 02:58, поточне завантаження **0010**.
- На апараті ввімкнено заводську галочку **«плаваюче відео»** (`persist.sys.has.float.video=true`) — власник увімкнув для
  доказу; вимкнути чи ні — його справа, коду вже байдуже. `persist.sys.maps = com.waze`.
- Вартовий дошки **знятий за наказом власника** («зніми слухач дошки»). Після компакту не підіймати без потреби.
- Звуком нічого не перевірялося — нічний борг звуку стоїть повністю, крім встановлення збірки й логера.

### Рішення власника цієї частини сесії (дослівно — не перепитувати)
| тема | слова власника | що це означає |
|---|---|---|
| Як говорити | «не розмовляти зі мною параграфами, люди ними не мислять» | жодних «§N», «T7» у розмові; у документах для агентів — можна |
| Звук уночі | «ніч — ніяких проб звуку не буде, відклади як нагальний борг в денний час… зафіксуй ясно і чітко» | блок «НАГАЛЬНИЙ ДЕННИЙ БОРГ — ЗВУК» у нічному зрізі + пам'ять `daytime-sound-debt-15-09` |
| Встановлення | «став поточне на магнітолу»; «став модуль, заребутеш зразу після встановлення» | зроблено 02:47 і 02:58 |
| Дж і git | «там не діє — це його проект [QFPlayer_fork]» | заборона Дж на git — лише wDSP і радіо |
| Дошка | «зніми слухач дошки» | знято |
| Звук під час роботи з UI | «та музика й так грає» | тапи в зони треків заставки дозволені |
| Логування дотиків | «логування зони тача зробити… прошивки для таких екранів нестандартні» | зроблено `d1c85d9` |
| Нижній рядок | спершу обрав «Менші кнопки», тоді: «а можна все і зразу зменшувати, підпис можна жати кегль, і трохи шрифт» | пропорційне стискання всього рядка, `c41812d` |
| «Гц» у списку | «менший шрифт пропорційно… або без пробілу (так можна), або прокрутку, як в пресетах» | стрілка не стискається + `singleLine` + прокрутка, `c41812d` |
| Анотація | «запам'ятай це десь, потім при роздачі допишеш в анотацію» | пам'ять `wdsp-next-release-notes-pending` |
| Навігація в заставці | «а нахріна воно взагалі перевірялося? Навігатор за замовчуванням — проп системи, а його пакет завжди он топ, навіть у піп вікні лончера» | прибрано три перевірки, `c8cf5d9` |
| Навігатор | «при встановленні\перевстановленні перевіряти, хто навігатор за замовчуванням, і додавати в наш чорний список, право юзера прибрати» | `a0745b6` |
| Пілюля | «зроби пілюлю на 640 теж — та я хз, я таких вузьких екранів давно не бачив» | `c2e46ef` |

### Зроблено (з доказами)
| коміт | що | як перевірено |
|---|---|---|
| `867be54` | нічний борг звуку записаний словами на початку нічного зрізу | — |
| `d1c85d9` | блок **SCREENSAVER** у звіті системи й у «Діагностика → Топологія екрана»: кожна умова зі значенням, найдовший простій, лічильник причин стримування, коли показано, останні 12 дотиків (координати у вікні й на екрані, розмір і місце вікна, числа зон, рішення). Правила — одна функція `whyHeldBack` | на апараті: показ над лаунчером через 60 с, тап вище середньої лінії записано; власник зберіг файл, блок на місці |
| `c41812d` | нижній рядок еквалайзера: блок «Підсилення» без ваги; `ui/RowFit` стискає весь рядок одним множником; частота `singleLine` + `TextScroller` | емуляцією (`wm size` + `dumpsys activity top -a`) на 1280/1024/800/768/640, скріни |
| `c8cf5d9` | з умов заставки прибрано `persist.sys.has.float.video`, `persist.sys.float_navi_bar`, `sys.qf.navi_state` | з галочкою «плаваюче відео»: над Waze стримано, над плеєром показано через 60 с |
| `a0745b6` | навігатор за замовчуванням (`persist.sys.maps`) додається у список винятків **раз за встановлення**; позначка `ss_navigator_added` у `wdsp_device_state` (поза бекапами) | у prefs `ss_blocked_pkgs = {com.waze}`, логер: `the default navigator com.waze is now in the list…` |
| `c2e46ef` | пілюля: кегль підписів і бічні відступи вкладок стискаються разом до 70 % | 640 — усі шість підписів цілі; 1280 без змін |
| HANDOFF `0126118` `ec6a8a5` `dd24dfa` `38ba5bc` | розділ «05:50–06:45 — скарги тестера з вертикальною панеллю» | — |

**Виміри нижнього рядка (dp).** До: блоку «Підсилення» дано / треба — 1280: 317/281, 1024: 222/281 («+» обрізано),
768: 149/236 (від «−» смужка), 640: 101/236 (ні числа, ні кнопок). Після: 1280 — один в один як було; 1024 — повний
розмір; 800 — ×0.98; 768 — ×0.94 (кнопки 45); 640 — ×0.73 (кнопки 35, «100 Гц» цілий).

### Доведено (НЕ переміряти)
1. **Галочка «плаваюче відео» — налаштування, не стан вікна.** Увімкнено → `persist.sys.has.float.video=true` одразу, у
   списку вікон жодного відео, лаунчер спереду, заставка з тієї секунди стримана щотіку (06:44:26). Де галочка є — заставки
   не було ніколи. У фреймворку це `ConfigInfoConstant.SET_CHECKBOX_NAME_MAP`.
2. **`sys.qf.current.activity` називає навігатор, навіть коли він у вікні лаунчера**: `com.waze/.MainActivity`, при цьому
   resumed і Waze, і лаунчер.
3. **`persist.sys.maps` — навігатор за замовчуванням**: так його читає сам фреймворк (`QFSleepWakeup` — відновлення після сну,
   `QFKeyEvent.isFreeformTask`). «Не задано» там — `nothing` / `map_not_config`.
4. **Пастка ваги в рядку**: блок з `layout_weight`, у якому підпис `wrap_content` стоїть першим, віддає кнопкам лише
   залишок — цифри вище. Коментар у розмітці стверджував протилежне.
5. **Тема ставить підписам і числам рядка 16 sp під час роботи** (`MainActivity`, прохід кольорів, списки
   `secondaryLabels` / `dataValues`) — розмір із розмітки (11–13 sp) не той, що на екрані.
6. **Стрілка Material-списку займає 52 dp** (`EndCompoundLayout`), усередині — 48 dp кнопка.
7. **На апараті власника вікно заставки = весь екран** (1280×720 у 0,0): рядок дотику з `d1c85d9`.
8. **Файл «Топології екрана»** зберігається в `/sdcard/Download/wDSP/wdsp_screen_topology_<версія>_<час>.txt`.
9. **Падіння 13.09 у звіті** (`IndexOutOfBoundsException` в `updateFmVisualizer`) — з експерименту відтворення, у коді
   виправлено й описано.
10. **Логер v2 на апараті**: `wDSP_bootlog: N more lines like this…` є; рут на завантаженні спитано один раз
    (`start: GRANTED`); `boot: resuming after a restart is off` (вмикач за замовчуванням вимкнений).
11. **На вертикальній панелі (Tesla) заставка стає правильно** — у смузі між верхнім баром і нижньою панеллю
    клімату, і її кнопки працюють як треба (підтвердження власника тієї панелі через Костянтина, 20.09.2026).
    Тобто зони, пораховані від повного екрана, лягають куди треба: вікно оверлея — на весь екран
    (`FLAG_LAYOUT_NO_LIMITS`), а бари прошивки просто намальовані поверх нього, тому координати дотику —
    екранні. ❓ Цифри з блоку дотиків того апарата не звіряли: файл «Топології екрана» звідти не приходив.

### Під питанням (не доведено — не видавати за факт)
1. **Чи в тестерів заставку блокували саме галочки.** Найімовірніше, але з їхнього апарата не підтверджено — чекаємо
   файл «Топології екрана» (прохання — в анотацію).
2. **`persist.sys.float_navi_bar`** — у фреймворку того самого типу, на апараті не вмикали. Коду вже байдуже.
3. ✅ **Знято 20.09.2026:** власник вертикальної панелі підтвердив — заставка будується правильно й кнопки на ній
   працюють. Див. «Доведено», пункт 11.
4. **Кнопки 35 dp на 640**: власник 12.09 казав, що 36 dp «уніформно і невлучно»; тепер сам просив стискати все. Око
   власника на 640 ще не бачило.
5. **Рядок на 1280 оком**: геометрія як була (скрін), але власник свіжу збірку на ньому не оцінював.
6. **Інші вкладки на вузьких вікнах** (Тонкомпенсація, Затримки, Інше, GALA) — `RowFit` лише в рядку еквалайзера; не міряно.
7. **«Налаштування» в пілюлі на 640 торкається краю** — ціле, але впритул.
8. Решта — з нічного зрізу (тихий бас на 31.5 Гц, крок гейна саба, 44.1/48 кГц, автовідновлення, пароль білого списку,
   ризик «чужий на вході», стара калібровка мікрофона).

### Тести, які винні (по порядку важливості)
**Вдень, зі звуком (власник):** весь нічний борг звуку — рут без тостів, вимір салону без замка, мікрофон (перше
вмикання, чужий на вході), відносний спектр, нове калібрування мікрофона → новий вимір салону → розовий шум проти кривої,
тихий бас на 31.5 Гц, автовідновлення плеєра, кнопка білого списку сну, логер у шторм HAL, радіо в режимі мікрофона, дзвінок
гасить заставку, ціна кроку гучності, саб після еквалайзера, **проба каналу вище 22 кГц** (код не написано). Рядки логера —
у таблиці тестів нічного зрізу. Пам'ять `daytime-sound-debt-15-09`.

**Без звуку, на апараті власника:**
- прибрати Waze зі списку винятків у налаштуваннях заставки → заставка над Waze з'являється; перевстановлення (не поверх)
  → Waze знову в списку;
- оком: нижній рядок і пілюля на 1280 після `c41812d`/`c2e46ef`; за бажанням — `wm size 640x720` / `768x782` (скидати!);
- заставка: тапи в зони треків і перетягування з країв → блок дотиків у «Топології екрана» показує рішення.

**Від тестерів:** файл «Топології екрана» після 2+ хв простою на іншому додатку (і після тикання заставки, якщо з'явиться).

### Борги коду й дизайну
1. **Проба каналу для верху свіпу** — у борзі звуку (нічний зріз).
2. Модель P2Bass у `DspResponse`.
3. Метадані системних плеєрів у заставці (протокол у базі Дж, дошка #539).
4. Межа AGC віджета — око власника.
5. Звірити крок гейна саба.
6. **Анотація наступної роздачі** — дописати: прохання до тестерів про звіт заставки; виправлення «галочки блокували
   заставку»; навігатор сам потрапляє у винятки й звідти прибирається; нижній рядок не ріже кнопки. Сім мов. Пам'ять
   `wdsp-next-release-notes-pending`.
7. `RowFit` на інші вкладки — лише якщо власник скаже (там не міряно).
8. Діагностичний `TICK` заставки пише в лог кожні 2 с (1784 рядки за ніч) — логер обрізає, але це шум; не чіпано.
9. Пуш 47 комітів — за наказом.

### Сесії Дж на цій машині
Без змін від нічного зрізу: **`931e1b40`** — рядки й 28 мов wDSP (цієї частини не залучалась, нових рядків немає);
**`60ce423d`** — прошивка/фреймворк QF, спершу її `references/`. Нове: `931e1b40` на прохання сесії `8dc041cc` зробила
переклади й інструменти локалей у **QFPlayer_fork** і сама закомітила там (`e55b4f6`, `20784fd`, `9c41cac`) — це її
проєкт, заборона на git там не діє.

### Мої помилки цієї частини (не повторювати)
- Говорив з власником «§5», «§6.1» — порушення давнього правила; записано повтор у `name-the-thing-not-the-index`.
- `cd` у Bash знову двічі змінив робочу теку сесії — лише абсолютні шляхи.
- `grep` у лозі логера ловив **власну команду**, яку `adbd` пише в лог («Tianyuan Current sh name:…») — три холості
  кола. Шаблон із класом символів (`show[n]`) + `grep -v adbd`.
- Тричі читав через `su` (логи, prefs застосунку) → тости Magisk у власника. Логи в `/data/local/tmp` читаються без рута.
- Перша версія `RowFit` запам'ятала кегль із розмітки й перебила 16 sp теми — спіймано вимірами, виправлено до коміту.
- `dumpsys activity top -a` одразу після `wm size` часто без нашої активності (перестворюється) — чекати 8–9 с, повторювати.
- Скріни при `wm size` з висотою > 720 обрізані — для картинки брати ту саму dp-геометрію через `wm size 707x720` +
  `wm density 147`; геометрію в dp — з дампу при точному розмірі. **Завжди `wm size reset` + `wm density reset`.**
- Уникав тапів у зони треків «щоб не запустити музику» — музика й так грала.
- Великий heredoc у Bash зламався на лапках — довгі тексти писати файлом через Write і дописувати `cat`.

### Інструменти й артефакти
- Скретчпад сесії `…\0079b58e-9830-44d5-bdef-a86693890553\scratchpad\`: `topology_061827.txt` (звіт власника з блоком
  заставки), `g_*`/`f_*`/`h_*.txt` (дампи геометрії до/після), `v640.png`, `v768.png`, `v1280.png`, `w640_row.png`,
  `p640_row.png`, `p1280_row.png`, `wdsp_tesla_eq*.png`, цей текст — `snapshot_15_09_0700.md`.
- Матеріали тестера: `C:\Users\kosty\AppData\Local\Packages\38833FF26BA1D.UnigramPreview_g9c9v27vpyspw\LocalState\0\documents\`
  (`топ.txt`, `wdsp_system_report_v0.4.9.8_vc25_20260914_194445.txt` і старіші звіти), фото
  `D:\Downloads\38833FF26BA1D.UnigramPreview_g9c9v27vpyspw!App\5278596819644523979_121.jpg`.
- Декомпільований фреймворк QF: `D:\UIS_android\qf_platform_framework\sources\sources\android\` (`constant/QFConstants.java`,
...
## 🔴🔴 20.09.2026, вечір — ЗРІЗ СЕСІЇ (читати ЦЕ першим)

> Попередній зріз «15.09 ~07:00» лишається дійсним у своїх частинах про заставку, рядок еквалайзера й борг звуку —
> тут вони не повторюються. 🗣️ З власником — словами про річ, без номерів розділів.

### Стан
- Гілка `kostyfmat_mod`. **Не запушено 58 комітів** (`origin/kostyfmat_mod` = `002c367`). Дерево чисте.
- **Код wDSP за ці п'ять днів не рухався**; робота власника йшла в сусідніх проєктах (QFPlayer_fork, QF_Companion,
  QF_NetworkBurst). Уся сьогоднішня праця — документи, дослідження й дві дрібні правки коду.
- На апараті власника — збірка з `c2e46ef` (15.09, 0.4.9.8), логер v2, завантаження 0033. Встановлено BitPerfect
  **v5.3**, його `service.sh` побайтово збігається з роздачею.
- Тестери досі на старій роздачі: у звітах 16.09 блоку заставки немає, тобто виправлення до них не доїхало.

### Зроблено сьогодні
| коміт | що |
|---|---|
| `ba2028e` | підтвердження власника вертикальної панелі: заставка стає правильно між барами, кнопки працюють — питання зон знято |
| `a09048c` | що з двох FYI Дж (#744/#745) стосується wDSP: майбутня підміна пакета радіо, приглушення в чіпі, сокира гучності |
| `36e90cc` | радіо впізнається за сімейством пакетів, а не за одним іменем: ім'я плеєра й тап по обкладинці заставки |
| `c33b458` | у репозиторій занесено п'ять знахідок 18.09 (adbd у лозі, трек в audio_flinger, Wi-Fi без втрати adb, флешка й «USB debug») |
| `3122f67` `551e8bd` | реєстр платформних «сокир» у документі про гучність + підрахунок парку: хаба AK немає в жодному з 21 звіту |
| `f0b23da` `5bf9d53` | три опитування поруч: v4.17 проти v5.1 проти апарата без модуля; чому та сама вада чується і «гучно», і «тихо» |
| `577df97` `e1aee9f` `0d744b9` | чому v5.3 вішає завантаження на UIS8581; що знято з підозр у мовчазному TTS; сторож гучності проти приглушення |
| `8a49238` | головне: платформа змішує по драбинці від «типової» гучності, а при коефіцієнті 90+ не приглушує взагалі |

Повна картина з доказами, варіантами рішень і переліком того, чого бракує — `.agents/RESEARCH_AUDIO_MIXING_AND_BITPERFECT.md`
(прив'язано в `INDEX.md`).

### Що зібрано для наступного кроку
- **Еталон 7862**: заводське дерево `D:\Release\update(2)\vendor\vendor` (27 файлів політик і маршрутів, вісім файлів
  параметрів AGDSP, `audio.primary.ums512.so`), звірене md5 із розділом vendor стенда. Копія знятого з апарата —
  `…\Temp\claude\qf_ref_7862\`.
- **Матеріали тестерів**: `…\Temp\claude\wdsp_survey_20_09\` — три опитування 06.09, опитування апарата, що не
  завантажився (07.09), розпакований модуль v5.3.
- **Чекаємо**: прошивку UIS8581 (власник качає, хост ріже швидкість). З неї — перелік `/vendor/etc`, політики,
  фірмові файли QF і сам HAL проти нашого еталона.

### Відкрите
1. Чи інша в 8581 робота зі звуком по суті — до прошивки.
2. Чому мовчить TTS — знімок треба брати **в момент** мовчання (стан `pcm3p`, власник, помилки HAL, живі треки).
3. Чи є витік пам'яті — за перші 30 хв нахилу немає; замір триває.
4. Чи робити приглушення самим замість платформної драбинки — питання власникові.
5. Старий борг звуку з 15.09 стоїть повністю (проба каналу вище 22 кГц так і не написана).

## 🔴 20.09.2026, ніч — доповнення до зрізу (свіп, еталонний документ, ГАЛА, заставка)

### Свіп із новою математикою — зроблено, результат розібрано
Власник прогнав калібрування мікрофона й вимір салону на стенді (звіт і запис — `…\Temp\claude\meas_20_09\`).
Порівняння з виміром 13.09 дало відповідь, чому новий Harman «гірший за старий»:

- **Верх.** До 15.09 канали усереднювалися **в дБ**, і мертвий лівий передній (на 12.5 кГц −31 дБ, на 20 кГц −41 дБ
  відносно правого) тягнув верх кривої вниз на 11–20 дБ. Harman це «виправляв» підйомом +4 +4 — саме він подобався
  власникові. Після переходу на усереднення **по потужності** (перевірено живим розовим шумом 14.09) верх більше не
  провалюється, і назовні вилізла друга помилка: **таблиця монтажу мікрофона** сліпо додає **+3.5 / +7.0 / +10.0 дБ**
  на 8 / 12.5 / 20 кГц. Вимір цього не підтверджує (правий канал приходить на 20 кГц лише на 5.5 дБ нижче середини),
  тож синтез вважає салон яскравим і ріже верх на 2–4 дБ. **Дві помилки роками гасили одна одну.**
- **Низ.** Компенсація мікрофона вперлася **рівно в стелю формули** шляху: +52.8 дБ на 20 Гц при стелі 52.8, +40.5 при
  41.1. Це насичення, а не вимір: на стенді немає приросту тиску салону, тому брак низу самого динаміка списується на
  капсуль. Наслідок — «крива салону» +6.6 / +8.7 на 20 / 31.5 Гц і жодного підйому баса в пресеті.
- Решта виміру здорова: передні канали чутно прямо (SNR 36–52 дБ), затримки знайдено, полярність зійшлася, тил на
  стенді не підключений — програма чесно написала «не чути».

**Рішення власника ще немає**, дві правки чекають його слова:
1. насичення вважати невідомим (нуль + рядок у звіті), 2. верх таблиці монтажу застосовувати лише за підтвердженням
у вимірі. Плюс його ж зауваження: **гучність виміру 16 може перевантажувати MEMS-капсуль** — спробувати меншу
(еталонний документ прямо каже про нелінійність MEMS на високому тиску).

### Еталонний документ про сліпе калібрування — прочитано
`D:\Downloads\Алгоритмічне та апаратне калібрування мікрофона та акустики без еталона.pdf`. Виписано в
`RESEARCH_AUDIO_MIXING_AND_BITPERFECT.md`, розділ 4: головна теза (мікрофон — єдине спільне для всіх каналів) і
таблиця розбіжностей із нашим кодом — оцінка по «найкращому каналу» замість спільної частини, фіксовані 6 дБ/окт
нижче 80 Гц замість 12 дБ/окт від розміру салону, відсутність перевірки «провал у всіх каналах = мікрофон», сліпа
таблиця монтажу замість теоретичної огинаючої, вікно замість оцінки RT60.

### Зроблено в коді цієї ночі
| коміт | що | перевірено |
|---|---|---|
| `72117c6` | **ГАЛА більше не лишає +1 після кожного гальмування**: коли підсилення падає в нуль, підняту самою ГАЛА гучність знімає командою, а чужу зміну, як і раніше, відстежує. Раніше нуль підсилення приймався за нову базу, і крок лишався назавжди | зібрано, встановлено; **на дорозі не перевірено** |
| `0e9ccb0` | **Заставка знімається, щойно під нею змінилася активність** (кнопка Mode, «недавні»); поки вона на екрані, опитування раз на 0.5 с замість 2 с | на апараті: з'явилася 23:29:32, знялася 23:29:47 через зміну додатка під нею |
| `df3e0ba` | еталонний документ у дослідження | — |
| `d3902b2` | `.agents/NEXT_STEPS_NOW.md` — жива нотатка «де зупинився», щоб компакт нічого не коштував | — |

На апараті стоїть збірка з усіма сьогоднішніми правками (встановлено 23:26), логер працює, завантаження 0034.

### Відкрите на завтра
1. Рішення власника по двох правках виміру (насичення / таблиця монтажу) і проба меншої гучності виміру.
2. Перевірити ГАЛА на дорозі (розгін — гальмування — рівень має повертатися).
3. Прошивка UIS8581 — качається, під неї готовий еталон 7862.
4. Старий борг звуку з 15.09 (проба каналу вище 22 кГц) — не починався.
5. BitPerfect — у боргах за наказом власника.

## 🔴 21.09.2026 — дві правки виміру зроблено, і що з документацією про мікрофон

### Наказ власника
«перевір дошку, читай зріз, і роби обидві правки виміру» — тобто ті дві, що з ночі чекали його слова.
Далі, тим самим ранком: «все що стосується повідомлень Дж щодо wDSP — твоє» (звіти Дж про заставку й
`sys.qf.current.activity` переходять до мене) і «я не знаю, чи ти це пам'ятаєш, але ми вже мікрофоном
слухали рожевий шум… добилися результатів, коли спектр мікрофона став показувати рівну АЧХ».

### Зроблено: обидві правки виміру (коміт «A saturated microphone estimate is unknown…»)
1. **Насичення = невідомо.** Смуга, чий дефіцит дійшов до межі того, що може вхідний тракт мікрофона,
   більше не записує цю межу як вимір: лишається цифра конструктиву, а звіт пише
   `Mic response UNKNOWN at … Hz`.
2. **Верх матриці — лише за підтвердженням.** `+3.5 / +7.0 / +10.0` на 8 / 12.5 / 20 кГц обмежені тим,
   що видно в огинаючій найкращого каналу, тобто спільною для всіх каналів частиною (перевірка з
   еталонного документа: провал у всіх = мікрофон, в одному = кімната). Звіт пише
   `Mounting treble TRIMMED at … Hz`.

Перевірено не збіркою, а числами: хостова проба (`test_miccal.cpp`, WSL g++) на огинаючій, зібраній із
звіту власника 20.09 — крива `+52.8 +40.5 +24.1 +13.6 +5.0 … +3.5 +7.0 +10.0` стала
`невідомо +40.6 +24.2 +13.6 невідомо … +1.7 +1.5 +1.5`. Повна таблиця й пояснення —
`ROOM_CALIBRATION.md`, розділ 24-ter.

**Третій свідок, про який нагадав власник.** Живий рожевий шум 14.09 (`compare_modes.py`, мікрофон
проти розрахункового) після переходу на усереднення по потужності дав розбіжність на верху **+0.9 /
+1.4 / +2.1 дБ**. Незалежний від свіпу метод каже те саме, що й сьогоднішнє обрізання: дефіциту в
7–10 дБ угорі немає. Унизу той дослід мовчить (20 Гц SNR −3.9) — що й означає «невідомо».

### Документація про конструктив і розміщення мікрофона — де вона лежить
Власник спитав, чи не розчинилося це в компактах. Не розчинилося:
- `ROOM_CALIBRATION.md` розділ **24** — резонатор Гельмгольца в отворі 1.5–2 мм, вхідний ФВЧ магнітоли
  (0.47–1.0 мкФ на 2.2 кОм + другий порядок у голосовому DSP), лінійність голого капсуля за
  даташитами, матриця чотирьох конструктивів, теорема розділення одного свіпу, чистота опори 200–500 Гц;
- розділ **24-bis** — що з цього в коді (виправлено 21.09: він досі казав, ніби матриці в коді немає);
- розділ **24-ter** — сьогоднішні межі матриці;
- розділи **2 («це отвір, а не капсуль»)**, **5 («де стоїть мікрофон — вирішує, чого вартий вимір»)**,
  **5-bis** — розміщення;
- `RESEARCH_AUDIO_MIXING_AND_BITPERFECT.md` розділ **4** — еталонний PDF про сліпе калібрування й
  таблиця розбіжностей із нашим кодом.

### 🔴 Знайдено дорогою, чекає слова власника
Пік Гельмгольца тепер лікують **двоє одночасно**: матриця в кривій мікрофона (смуга 11, `−5.0` дБ для
Pinhole) і стара милиця після синтезу (`analyzeAcousticsAndSynthesize` додає смугам 11–12 по два кроки,
до +4 дБ). Поки матриці не було, милиця була єдиним захистом голосу в музиці; тепер це схоже на
подвійну компенсацію одного отвору. Нічого не чіпав — вирішувати на слух у салоні.

### Далі за списком
1. Перемикач «Накривати статус-бар» у заставці (наказ власника через Дж, дошка #747).
2. ГАЛА перевірити на дорозі.
3. Проба виміру на гучності, меншій за 16 (перевантаження MEMS) — коли буде день і дозвіл на звук.
4. Прошивка UIS8581, коли докачається. Борг звуку з 15.09. BitPerfect — у боргах.

### 🔴 Того ж ранку: правило «одна річ — одна функція» проти подвійного лікування отвору
Власник, прочитавши про подвійну компенсацію: «у нас є правило — джерелом правди є метод\функція або
максимум один клас на всю програму… і чи не саме те, що лікують двоє, має авторство горба на середніх?»

Майже. Автор горба — **сліпий мінус у самій матриці**, а не другий лікар. Матриця знімає 5 дБ на
3150 Гц під резонанс отвору, якого на цьому апараті немає (свіп читає 3150 на децибел нижче
середини). Смуги 10–12, опущені на −1.5 / −5.0 / −1.0, дали криву салону −2.9 / −6.6 / −2.5, і
Auto-EQ відповів підйомом 2–5 кГц. Милиця після синтезу навіть не спрацювала — вона піднімає лише
зрізані смуги.

Зроблено (коміт «One place treats the microphone's mounting…»):
- три гілки конструктиву після синтезу **видалено**; конструктив живе тільки в кривій мікрофона.
  Розміщення (лобове скло) лишилося — це інше питання;
- перевірка виміром тепер в **обидва боки**: провал звіряється з огинаючою найкращого каналу, пік —
  з огинаючою найгіршого (спільне для всіх = мікрофон). Друга огинаюча — `NaN`, поки впевнених
  каналів менше двох;
- калібрування читає `effectiveMicBody(...)`, а не преференс напряму: звіт і калібрування давали
  різні відповіді на одне питання.

На числах власника крива Pinhole: `невідомо +40.6 +24.2 +13.6 невідомо … 0.0 0.0 0.0 +1.7 +1.5 +1.5`.
⚠️ На апараті в пам'яті ще стара крива — потрібен новий свіп калібрування (день, дозвіл на звук).

### 🧭 Конструктив і розміщення зведено в один клас — `MicProfile`
Наказ: «зібрати логіку в купу… орієнтуючись на те, що сказав юзер». Було чотири місця на два факти,
стало одне — `wdsp_app/src/main/java/com/radiorubka/wdsp/MicProfile.java`. Порядок довіри записаний у
самому класі: **що сказала людина → що місце означає поза сумнівом → що підтвердив вимір** (третє
обмежує перші два й ніколи не вигадує свого). Повна карта «яке питання — де відповідь» у
`ROOM_CALIBRATION.md`, розділ 24-quater.

Переїхало: таблиця кривих монтажу з `sweep.cpp` (нативний бік тепер отримує криву й не тримає копії),
списки місць і конструктивів із їхніми префами, таблиця висот, крапка на плані, висновок «магнітола =
отвір», слова для звіту, і єдине правило розміщення (лобове скло) як `applyPlacementLimits`.
`RoomMeasurement` −316 рядків; `SettingsActivity` питає той самий клас, тож екран і калібрування
більше не розійдуться в тому, що за мікрофон стоїть. Поведінка не змінилася — хостова проба на
числах 20.09 дає ту саму криву смуга в смугу (коміт `e13f38a`).

⚠️ Відкрите вже не про місце в коді, а про математику: спільна частина замість найкращого каналу;
якір низу від кузова замість фіксованих 6 дБ/окт; теоретична огинаюча замість таблиці на верху.

### 🌙 Закриття дня 21.09 — що власник назвав боргом і що перевіряємо завтра
Сесія закрита за його словом: «на сьогодні досить». Усе сьогоднішнє **перевірено офлайн** на його ж
числах 20.09 (харнес `test_miccal.cpp`, тепер у репозиторії) і **не стояло на апараті** — там досі
збірка від 20.09 23:26 зі старою кривою мікрофона.

**Завтра вдень:** свіп калібрування наново (стара крива сама не перерахується) і вимір салону, проба
меншої гучності за 16, живий звук і враження власника. Окремо — **поміряти те, що ніколи не мірялось**:
підсилення саба (стала +2 дБ для Harman у `synthesizeAutoEq16`) і стелю підйому (+3 дБ до округлення,
індекс 8 — саме в неї впхався синтез на 125 Гц). Власник: «ось це і поміряємо, і подумаємо завтра».

**Два борги, названі вголос:**
1. **Запуск плеєрів після ребуту і після сну не працює** — мало б піднімати те, що грало до
   засинання/вимкнення (`PlayerResume`: запам'ятовує на `READY_GO_SLEEP`/`ACC_OFF`, піднімає на
   `ACC_ON` або протягом 3 хв від старту ядра). Перевіряти по ланцюгу: чи ловиться подія → чи
   переживає процес сон → чи доходить PLAY до media button receiver плеєра → чи не заважає білий
   список сну платформи (його застосунок не читає; Settings лише відкриває заводський екран).
2. **Перемикач «Накривати статус-бар»** у розділі заставки — анатомія вади й вимога у #747 на дошці.

Стан репозиторію: дерево чисте, не запушено ~85 комітів, гілка `kostyfmat_mod`.

### 🌙 Ніч 21→22.09: обидва борги закрито в коді, збірка стоїть на апараті
Власник: «так постав, щоб завтра все було готове. Але спочатку розберися з боргом — плеєри не
запускаються. Причину не знаю: не те читаєш, не записуєш, не так запускаєш, наприклад без контексту,
а Спотіфай без контексту й прицільного інтенту в нього самого не запустиш, як і будь-який інший
плеєр, бо якщо ніхто ще не зайняв медіасесію, то простий input key полетить у пустоту».

Діагноз підтвердився **його ж логером**, обидві половини були зламані.
- **Записування.** Завантаження 0039–0042: «after restart: nothing was playing». Причина: на
  завантаженні питалося «чи ще грало?», а останнє перед вимкненням — платформа ставить на паузу.
  Тепер пауза звіряється з моментом старту машини (`PAUSE_BEFORE_BOOT_MS`): у межах двох хвилин до
  нього — це вимкнення, раніше — людина.
- **Запуск.** Завантаження 0034: «asked …youtube.music … through its media button receiver» →
  «is NOT playing 6000 ms later». «Є приймач медіакнопки» вважалося успіхом, тож браузер-сервіс —
  єдина сходинка, що піднімає мертвий процес, — не пробувався ніколи. Тепер драбинка з перевіркою:
  сесія → браузер-сервіс → **відкрити сам застосунок** → сесія ще раз → медіакнопка.

**Перевірено на дроті (0043):** `asked com.kostyamat.fmradio to play through opening the app itself`
→ через 5 с `is playing`. Механізм працює. ⚠️ **Але це було радіо, а радіо чіпати не можна** — і це
власна вада, знайдена тим самим прогоном: відмова жила лише в гілці ACC_OFF, а шлях завантаження її
не знав. Виправлено там, де записується факт: радіосесія більше не стає ціллю, а перехід на радіо
стирає запам'ятоване. Радіо на апараті зупинено того ж хвилиною (`sys.qf.radio.status=false`,
гучність апарата стоїть 1).

**Перемикач «Накривати статус-бар»** (#747) зроблено, типово вимкнено: вікно — і позичене в смужки, і
власне — починається під баром і на стільки ж коротше, бекдроп від `usableTop`. Оверлей не
перехоплює того, чого не накриває.

На апараті збірка 22.09 ~04:5x (versionCode 25). ⬜ Обидва борги чекають перевірки **живим звуком і
очима** — це день.
